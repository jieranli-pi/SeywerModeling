...
for i =1:18
    ...
    water_depth=1;%water depth inside of pipe =30cm
    %%%%%%%%%%%%%%%%parameter input end

    %variable
    leakage_start=(i-1)*360/18;
    leakage_end=leakage_start+360/18;
    name=append('between',string(leakage_start),"&",string(leakage_end));
    %transpose
    width=width/onegrid;
    ...
    disp('calculation')
    %visualise T matrix
    plot_trans(S_2DT,name)
    
    %visualise potential matrix
    plot_phi(S_2Dphi,name)
    
    flow_contour(S_2Dphi,electrode_center,electrode_radius,name)
    disp('plotting')
    save(append(name,'current.mat'),'Current','-v7.3')
    close
    clear
...
savefig(append('transmissitivity',name,'.fig'))
end
...
savefig(append('Potential.',name,'.fig'))
end
...
axis equal
hold off
savefig(append('flowcontour',name,'.fig'))
end
