...
for i = 1:size^2+size
    ...
    %entry
    elseif i<size^2+size+1 && i>size^2+size+1-5-1%length=5
        Coefficient_matrix(i,:)=BRmultientry(Coefficient_matrix(i,:),i,size,T_matrix,T_wire,5,1,size*(size+1)-5+1);
    %End
    elseif i-1==size
        Coefficient_matrix(i,:) = fixed(Coefficient_matrix(i,:));
    %Topleft corner
    elseif i==size+1
        Coefficient_matrix(i,:) = TLcorner(Coefficient_matrix(size,:),i, size);
    %Topright corner
    elseif i==2*size
        Coefficient_matrix(i,:) = TRcorner(Coefficient_matrix(i,:),i, size);
    %Botleft corner
    elseif i==size^2+1
