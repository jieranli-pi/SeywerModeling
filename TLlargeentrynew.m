...
for i = 1:size^2+size
    ...
    %entry
    elseif i-size<5+1%length=5
        Coefficient_matrix(i,:)=TLmultientry(Coefficient_matrix(i,:),i,size,T_matrix,T_wire,5,2,size+1);
    elseif i-2*size<5+1 && i>size*2%length=5
        Coefficient_matrix(i,:)=TLmultientry(Coefficient_matrix(i,:),i,size,T_matrix,T_wire,5,2,size+1);
    %End
