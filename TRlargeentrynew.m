...
for i = 1:size^2+size
    ...
    %entry
    elseif i-2*size<1 && i-2*size>-5%length=5
        Coefficient_matrix(i,:)=TRmultientry(Coefficient_matrix(i,:),i,size,T_matrix,T_wire,5,2,2*size);
    elseif i-3*size<1 && i-3*size>-5%length=5
        Coefficient_matrix(i,:)=TRmultientry(Coefficient_matrix(i,:),i,size,T_matrix,T_wire,5,2,2*size);
    %End
